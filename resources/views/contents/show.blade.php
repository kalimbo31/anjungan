...
<section class="min-w-full bg-green-600 pt-16">
    <div class="max-w-screen-xl mx-auto">
        <h1 class="pt-10 pb-5 px-16 font-semibold md:text-3xl sm:text-2xl text-xl text-gray-100">{{ is_object($item) && $item->category ? $item->category->name : (is_string($item) ? $item : '') }}</h1>
    </div>
</section>
    ...
    <div class="grid grid-cols-10 max-w-screen-xl mx-auto lg:space-x-3 lg:px-10 px-3">
        <div class="lg:col-span-7 col-span-10 p-5 shadow-md rounded-md mx-auto bg-white mt-3 lg:mb-10 w-full">
            @if (is_object($item))
                ...
                <div class="text-base font-poppins text-slate-800 leading-7 w-full space-y-3 mb-4">{!! $item->body !!}</div>
                @if($item->file)
                @php
